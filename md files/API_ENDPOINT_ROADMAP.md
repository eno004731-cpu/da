...
| `POST` | `/api/auth/email-verification/request` | Verification | `Implemented` | `Auth required` | Bearer token, без body | `VerifyEmailResponse` | `auth_service/legal_website` | Контроллер уже есть, flow отправляет событие на notification-service. |
